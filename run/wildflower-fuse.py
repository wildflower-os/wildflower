...
BLOCK_SIZE = 512
KERNEL_SIZE = 4 << 20 # 4 MB
SUPERBLOCK_ADDR = (KERNEL_SIZE // BLOCK_SIZE) * BLOCK_SIZE
BITMAP_ADDR = SUPERBLOCK_ADDR + 2 * BLOCK_SIZE
ENTRY_SIZE = (1 + 4 + 4 + 8 + 1)

class WildflowerFuse(LoggingMixIn, Operations):
    chmod = None
    chown = None
    readlink = None
    rename = None
    ...
    def __init__(self, path):
        ...
        assert superblock[0:8] == b"Wildflower FS" # Signature
...
if __name__ == '__main__':
    ...
    fuse = FUSE(WildflowerFuse(args.image), args.mount, ro=False, foreground=True)
