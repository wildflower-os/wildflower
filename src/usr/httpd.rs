...
impl Response {
    pub fn new(req: Request) -> Self {
        ...
        headers.insert(
            "Server".to_string(),
            format!("WildflowerOS/{}", env!("CARGO_PKG_VERSION")),
        );
        Self {
            req,
            buf: Vec::new(),
...
fn get(req: &Request, res: &mut Response) {
    if fs::is_dir(&res.real_path) && !req.path.ends_with('/') {
        ...
        res.headers
            .insert("Location".to_string(), format!("{}/", req.path));
        ...
        if not_found {
            if let Ok(mut files) = fs::read_dir(&res.real_path) {
                ...
                res.mime = "text/html".to_string();
                res.body
                    .extend_from_slice(format!("<h1>Index of {}</h1>\n", req.path).as_bytes());
                        ...
                        let res = socket.recv(|chunk| {
                            ...
                            if let Some(req) = Request::from(addr, &buf) {
                                ...
                                match req.verb.as_str() {
                                    "GET" => get(&req, &mut res),
                                    "PUT" if !read_only => put(&req, &mut res),
                                    "DELETE" if !read_only => delete(&req, &mut res),
                                    _ => {
